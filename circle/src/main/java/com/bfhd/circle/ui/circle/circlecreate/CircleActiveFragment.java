...
import com.bfhd.circle.databinding.CircleFragmentCircleCreateActiveBinding;
 ...
 * 兴趣圈
 * */
public class CircleActiveFragment extends CommonFragment<CircleViewModel, CircleFragmentCircleCreateActiveBinding> {
    ...
    protected int getLayoutId() {
        return R.layout.circle_fragment_circle_create_active;
    }

    public static CircleActiveFragment newInstance(String mutid, String mCircleID) {
        CircleActiveFragment detailFragment = new CircleActiveFragment();
    ...
    public void create() {
        ...
        if (TextUtils.isEmpty(mUtid)) { // 创建
            mViewModel.createCircle(mCircleCreateVo, "2");
        } else { // 编辑
            mCircleCreateVo.utid = mUtid;
            mViewModel.editCircle(mCircleCreateVo, "2",mCircleID);
        }
