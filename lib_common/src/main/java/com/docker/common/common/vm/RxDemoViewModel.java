...
public class RxDemoViewModel extends NitCommonContainerViewModel {
    ...
    public void rxConcatMap(String memberid, String uuid) {
        ...
        openService.tttt1(paramMap).doOnNext(new Consumer<BaseResponse<List>>() {
            @Override
        ...
        }).concatMap(new Function<BaseResponse<List>, ObservableSource<?>>() {
            @Override
            public ObservableSource<?> apply(BaseResponse<List> listBaseResponse) throws Exception {
//                RadioMenuVo vo = listBaseResponse.getRst().get(0);
                paramMap.put("lession_dubbing_id", "2");
                return openService.tttt2(paramMap);
            }
                ...
                new Consumer<BaseResponse>() {
                    ...
                    public void accept(BaseResponse baseResponse) throws Exception {
                        List radioLessonVos = (List) baseResponse.getRst();
//                        LogUtils.e(radioLessonVos.get(0).getName());
                        mContainerLiveData.setValue(radioLessonVos);
                    }
                });
    ...
    public void rxMerge(String memberid, String uuid) {
        ...
        paramMap.put("memberid", memberid);
        paramMap.put("uuid", uuid);
        Observable.merge(openService.tttt1(paramMap), openService.ttttt3(paramMap))
                .observeOn(AndroidSchedulers.mainThread()).subscribeOn(Schedulers.newThread())
                .subscribe(new Consumer<BaseResponse<? extends List>>() {
                    @Override
                    public void accept(BaseResponse<? extends List> baseResponse) throws Exception {
                        mContainerLiveData.setValue(baseResponse);
