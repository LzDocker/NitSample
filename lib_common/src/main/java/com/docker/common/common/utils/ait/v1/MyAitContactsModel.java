...
public class MyAitContactsModel {

    // 已@ 的成员
    private Map<String, MyAitBlock> aitBlocks = new HashMap<>();
    ...
    public void addAitMember(String account, String name, int type, int start) {
        MyAitBlock aitBlock = aitBlocks.get(account);
        if (aitBlock == null) {
            aitBlock = new MyAitBlock(name, type);
    ...
    public List<String> getAitTeamMember() {
        ...
        while (iterator.hasNext()) {
            ...
            MyAitBlock block = aitBlocks.get(account);
            if (block.aitType == MyAitContactType.TEAM_MEMBER && block.valid()) {
    ...
    public MyAitBlock getAitBlock(String account) {
    ...
    public String getFirstAitRobot() {
        ...
        while (iterator.hasNext()) {
            String account = iterator.next();
            MyAitBlock block = aitBlocks.get(account);
            if (block.valid() && block.aitType == MyAitContactType.ROBOT) {
    ...
    public MyAitSegment findAitSegmentByEndPos(int start) {
        Iterator<String> iterator = aitBlocks.keySet().iterator();
        while (iterator.hasNext()) {
            String account = iterator.next();
            MyAitBlock block = aitBlocks.get(account);
            MyAitSegment segment = block.findLastSegmentByEnd(start);
            if (segment != null) {
                return segment;
            }
        }
